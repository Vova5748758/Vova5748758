**RUSSIAN 
советую использовать pastebin достаточно понятен в использовании 
чтобы сделать loadstring сыллку используйте этот шаблон 

--------

loadstring(game:HttpGet("ваша сыллка"))()

--------
подробное на https://youtu.be/d8mJAo8KXyE?si=sVjBjOiUHN3Xd2RH (НЕ АВТОР)

пишите вопросы на hffh65910@gmail.com
скрипт GUI
-----------
local library = loadstring(game:HttpGet("https://raw.githubusercontent.com/bloodball/-back-ups-for-libs/main/turtle"))()
local Window = library:Window("Window") --you can copy and install it so that there are two floating window
Window:Button("Button", function() --Button 
print("click")
end)
Window:Box("TextBox", function(text, focuslost)--TextBox
        print("TextBox")
end)

Window:Text("Text", function(text, focuslost)
    text = "Text"
end)

--скрипт от CDTY0898 
-----------

можите использовать другие библиотеки --  https://github.com/topics/roblox-ui-lib  --


**ENGLISH 
I recommend using pastebin, it's pretty easy to useI recommend using pastebin, it's pretty easy to use
to make a loadstring link, use this template

--------

loadstring(game:HttpGet("your link"))()

--------

more details at https://youtu.be/d8mJAo8KXyE?si=sVjBjOiUHN3Xd2RH (NOT THE AUTHOR!!!!)

details on hffh65910@gmail.com
script GUI

-----------
local library = loadstring(game:HttpGet("https://raw.githubusercontent.com/bloodball/-back-ups-for-libs/main/turtle"))()
local Window = library:Window("Window") --you can copy and install it so that there are two floating window
Window:Button("Button", function() --Button 
print("click")
end)
Window:Box("TextBox", function(text, focuslost)--TextBox
        print("TextBox")
end)

Window:Text("Text", function(text, focuslost)
    text = "Text"
end)

--script from CDTY0898 
-----------

you can use other libraries -- https://github.com/topics/roblox-ui-lib --
